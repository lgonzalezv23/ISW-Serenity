...
$sql_citas = "SELECT c.id, c.fecha, h.dia_semana, h.hora_inicio, h.hora_fin, u.nombre AS usuario_nombre, u.apellidos AS usuario_apellidos 
              FROM citas c
              JOIN horarios h ON c.horario_id = h.id
              JOIN users u ON c.usuario_id = u.id
              WHERE h.especialista_id = ?
              ORDER BY c.fecha DESC";
$stmt_citas = $conn->prepare($sql_citas);
$stmt_citas->bind_param("i", $especialista_id);
$stmt_citas->execute();
...
<body>
    ...
    <div class="container">
        <div class="titulo">Citas Agendadas</div>
        <div class="content">
            <div class="citas">
                <table>
                    <thead>
                        ...
                        <?php if ($result_citas->num_rows > 0): ?>
                            <?php while ($row = $result_citas->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($row['usuario_nombre'] . " " . $row['usuario_apellidos']); ?></td>
                                    <td>
                                        <form method="POST" action="cancelar_cita_especialista.php" style="display:inline;">
